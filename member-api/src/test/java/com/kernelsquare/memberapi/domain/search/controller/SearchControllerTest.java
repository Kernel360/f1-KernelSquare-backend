...
class SearchControllerTest {
	...
	void testSearchTechStacks() throws Exception {
		...
		//when & then
		mockMvc.perform(get("/api/v1/search/techs?keyword=j")
				.with(csrf())
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)
				.characterEncoding("UTF-8"))
			...
			.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
			.andExpect(jsonPath("$.code").value(SEARCH_TECH_STACK_COMPLETED.getCode()))
			.andExpect(jsonPath("$.msg").value(SEARCH_TECH_STACK_COMPLETED.getMsg()));
